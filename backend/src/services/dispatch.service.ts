...
export class DispatchService {
  ...
  findActiveByVehicle(vehicleId: number) { return this.rows.filter((item: any) => item.vehicleId === vehicleId && (item.status === DispatchStatus.Assigned || item.status === DispatchStatus.InProgress)); }
